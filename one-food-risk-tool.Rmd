...
template_impacts <- c("Food production",
                      "Human health",
                      "Natural environment",
                      "Climate change")
  ...
  ifelse(length(input$template_haz_bio) > 0,
         biological_template <- data.frame(Category = rep("Biological", 
                                                          length(input$template_haz_bio) *
                                                            length(template_impacts) *
                                                            length(template_creation$template_sector)),
                                           Hazard = sort(rep(input$template_haz_bio,
                                                        length(template_impacts) *
                                                        length(template_creation$template_sector))),
                                           Impact = rep(sort(rep(template_impacts,
                                                               length(template_creation$template_sector))),
                                                        length(input$template_haz_bio)),
                                           Sector = rep(template_creation$template_sector,
                                                             length(input$template_haz_bio) *
                                                               length(template_impacts)),
                                           Severity = NA,
                                           Probability = NA,
...
updated_data <- reactive({
  ...
  if(input$type == "Hazard category"){
    if(input$fill == "Sector"){
      hazard_scores$user[Category %in% input$category][, .(Score = sum(Score)), by = c("Sector", "Hazard")]
    ...
    if(input$fill == "Sector"){
      hazard_scores$user[, .(Score = sum(Score)), by = c("Category", "Sector")]
    } else if(input$fill == "Impact"){
...
of_plot <- reactive({
  if(input$type == "Hazard category" &&
    ...
    spider <- ggplot(data = updated_data(),
           aes(x = Category, y = Score, col = Sector, group = Sector,
               fill = Sector)) +
